...
int main(){
    ...
    long int n, i;
    FILE *file_pointer;
    file_pointer = fopen("./OUTPUT_graycode.txt", "w");
    ...
    for(n = 3; n <= MAX; n++){
        ...
        for (i = 0; i < power(2, n); i++){
            fprintf(file_pointer, "\n%ldth gray code in %ld bit is %010ld", i, n, int2int((long int) i ^ (i >> 1)));
        }
        end_i = clock();
        time_passed = ((double) (end_i - start_i) / CLOCKS_PER_SEC);
        fprintf(file_pointer, "\n\nTIME PASSED WHILE GENERATION OF %ld BITS OF GRAY CODE = %f", n, time_passed);
    }
    end = clock();
